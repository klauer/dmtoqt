...
class PropertySet:
    ...
    def addProperty(self, propname, typeval):
        ...
        if not found:
            logging.warn(
                "Variable %s not found on class %s" % (propname, self.classname)
            )
    ...
    with open(os.path.join(root, fname)) as f:
        ...
        for line in f:
            ...
            if m.group(1) == "beginObjectProperties":
                logging.info(
                    "Line %d: Found begin Object properties for class %s",
                    lineno,
                    classname,
                )
                continue
            if m.group(1) == "endObjectProperties":
                logging.info(
                    "Line %d: Found end object properties for class %s",
                    lineno,
                    classname,
                )
                break
            if m.group(1) == "beginScreenProperties":
                logging.info(
                    "Line %d: Found begin Screen properties for class %s",
                    lineno,
                    classname,
                )
                continue
            if m.group(1) == "endScreenProperties":
                logging.info(
                    "Line %d: Found end Screen properties for class %s",
                    lineno,
                    classname,
                )
                break
            logging.info(
                "Line %d: class %s: property %s => %s",
                lineno,
                classname,
                m.group(1),
                m.group(2),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Parse EDM source files for widget properties"
    )
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Output path (default=./widgets/edmprops)",
        default="widgets/edmprops",
    )
    ...
    if not os.path.isdir(args.output):
        print(
            (
                "Ouput path %s does not appear to be a directory; attempting to create"
                % args.output
            )
        )
        try:
