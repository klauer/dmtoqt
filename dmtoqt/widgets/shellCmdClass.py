...
class shellCmdClass(BaseWidget):
    ...
    def widgetUI(self, parent):
        ...
        if self.framework() == "EpicsQt":
            ...
            if self.isMenuButton():
                ...
                if (
                    "commandLabel" in self.widget.props
                    and len(self.widget.props["commandLabel"]) > 0
                ):
                    self.stringProperty(
                        elem, "text", self.widget.props["commandLabel"]["0"]
                    )
            ...
            if "closeDisplay" in self.widget.props and bool(
                self.widget.props["closeDisplay"]
            ):
    ...
    def menuEntries(self, elem):
        ...
        for key in list(self.widget.props["command"].keys()):
            itemelem = etree.SubElement(
                root, "Item", {"Name": self.widget.props["commandLabel"][key]}
            )
