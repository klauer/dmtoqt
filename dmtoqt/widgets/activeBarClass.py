    ...
    def widgetUI(self, parent):
        ...
        if self.framework() == "EpicsQt":
            ...
            if "orientation" in self.widget.props:
                self.enumProperty(
                    elem, "orientation", "QEAnalogIndicator::Bottom_To_Top"
                )
            ...
            if "border" in self.widget.props:
                ...
                self.colorProperty(
                    elem, "borderColour", {"r": 0, "g": 0, "b": 0, "a": 0}
                )
            ...
            if "fgAlarm" in self.widget.props:
                self.enumProperty(
                    elem, "alarmSeverityDisplayMode", "QEAnalogProgressBar::foreground"
                )
            if "indicatorAlarm" in self.widget.props:
                self.enumProperty(
                    elem, "displayAlarmStateOption", "QEAnalogProgressBar::WhenInAlarm"
                )
        else:  # caQtDM
            ...
            if "limitsFromDb" not in self.widget.props:
                self.stringProperty(
                    elem, "displayLimits", "EAbstractGauge::User_Limits"
                )
