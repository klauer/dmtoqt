    ...
    def widgetUI(self, parent):
        ...
        if self.widgetType() == "QESlider":
            if "orientation" not in self.widget.props:
                self.enumProperty(
                    elem, "orientation", "Qt::Horizontal", {"stdset": "0"}
                )
            ...
            if (
                found
            ):  # Even if vertical, property gets written as TicksAbove/TicksBelow
            ...
            if "fgColor" in self.widget.props:
                ...
                ss += " color: %s" % str(rgb)
            if (
                "bgColor" in self.widget.props
                and "useDisplayBg" not in self.widget.props
            ):
                if applyss:
                    ss += ";"
                else:
            ...
            if "savedValuePv" in self.widget.props:
                self.logger.warn(
                    'savedValuePv property set to "%s"; ignoring as there\'s no equivalent on Qt Analog Sliders'
                    % self.widget.props["savedValuePv"]
                )
