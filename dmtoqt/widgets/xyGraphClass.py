...
class xyGraphClass(BaseWidget):
    ...
    def widgetUI(self, parent):
        ...
        if (
            "showYAxis" not in self.widget.props
            and "showY2Axis" not in self.widget.props
        ):
            self.booleanProperty(elem, "axisEnableY", False)
        elif "showYAxis" in self.widget.props and "showY2Axis" in self.widget.props:
            self.logger.warn(
                'QEPlot "%s": Y and Y2 axes will be combined into one.'
                % self.widgetName()
            )
            # TODO: combine both y axes into one
            """ If either axis is auto-scaled, make this axis auto-scaled """
            if (
                "yAxisSrc" in self.widget.props
                and self.widget.props["yAxisSrc"] == "fromUser"
            ) and (
                "y2AxisSrc" in self.widget.props
                and self.widget.props["y2AxisSrc"] == "fromUser"
            ):
                ymin = 0
                ymax = 1
            ...
            if "yLabel" in self.widget.props and "y2Label" in self.widget.props:
                ylabel = (
                    self.widget.props["yLabel"] + " / " + self.widget.props["y2Label"]
                )
        ...
        if "yPv" in self.widget.props:
            ...
            for key, yPv in list(self.widget.props["yPv"].items()):
                ...
                self.colorProperty(
                    elem, "traceColor%d" % i, self.widget.props["plotColor"][key]
                )
                i += 1
                if i > 4:
                    self.logger.warn(
                        "#Y variables exceeds 4; remaining variable(s) will be ignored"
                    )
                    break
    ...
    def yaxisUI(self, elem, axisname):
        if (
            axisname + "AxisSrc" in self.widget.props
            and self.widget.props[axisname + "AxisSrc"] == "fromUser"
        ):
