    ...
    def widgetUI(self, parent):
        ...
        if len(alarmpvs) > 1:
            self.logger.warn(
                ...
                % (len(self.widget.stack))
            )
            alarmpvs.insert(0, alarmpv)
            self.stringListProperty(elem, "alarmPvs", alarmpvs)
        if len(vispvs) > 0:
            self.logger.warn(
                'Widget stack of %d widgets contains more than one visPv.  You must fix this manually in your .ui file.  The pvs will be listed in a property of the widget called "visPvs", along with the visMin/visMax properties for each.'
                % (len(self.widget.stack))
            )
        ...
        if self.widget.stackType() == "labels":
            ...
            if "fillColor" in self.widget.firstWidget.props:
                ...
                if applyBg:
                    ss = (
                        self.widgetType()
                        + " { background-color: %s; }" % self.colors.getRGB(bgcolor)
                    )
