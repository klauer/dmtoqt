...
class activeSliderClass(BaseWidget):
    ...
    def widgetUI(self, parent):
        ...
        if "savedValuePv" in self.widget.props:
            self.logger.warn(
                'savedValuePv property set to "%s"; ignoring as there\'s no equivalent on Qt Sliders'
        ...
        if (
            "controlAlarm" in self.widget.props
            or "indicatorAlarm" in self.widget.props
            or "bgAlarm" in self.widget.props
        ):
