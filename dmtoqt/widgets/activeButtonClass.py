...
class activeButtonClass(BaseWidget):
    ...
    def widgetUI(self, parent):
        ...
        if self.framework() == "EpicsQt":
            ...
            txt = ""
            if (
                "labelType" in self.widget.props
                and self.widget.props["labelType"] == "literal"
            ):
                if (
                    "offLabel" in self.widget.props
                    and self.widget.props["offLabel"] != ""
                ):
                    txt += self.widget.props["offLabel"]
                if (
                    "onLabel" in self.widget.props
                    and self.widget.props["onLabel"] != ""
                ):
                    if txt != "":
                        txt += "/"
            ...
            if txt == "":
                self.logger.warn(
                    "%s %s: EDM would fill labelText with pv state, but this tool cannot do that.  Attempting to use pv name; please examine this widget in Designer."
                    % (self.widgetType(), self.widgetName())
                )
    ...
    def widgetType(self):
        if self.framework() == "EpicsQt":
            if (
                ...
                and self.widget.props["buttonType"] == "push"
            ):
                return "QEPushButton"
            return "QECheckBox"
        if (
