...
class activeLineClass(BaseWidget):
    ...
    * Absent all of the above preconditions: a QEShape
    ...
    def widgetUI(self, parent):
        ...
        if self.widgetType() == "Line":
            ...
            if alarmpv:
                ...
                self.stringProperty(
                    elem, "channel", self.widget.props["alarmPv"], {"notr": "true"}
                )
            ...
            if int(self.widget.geometry["w"]) == 16:
                if self.widget.props["arrows"] == "to":
                    if int(self.widget.props["yPoints"]["0"]) < int(
                        self.widget.props["yPoints"]["1"]
                    ):
                        shape = "arrowDown"
                    else:
                        shape = "arrowUp"
                ...
                if self.widget.props["arrows"] == "to":
                    if int(self.widget.props["xPoints"]["0"]) < int(
                        self.widget.props["xPoints"]["1"]
                    ):
                        shape = "arrowRight"
                    else:
                        shape = "arrowLeft"
            ...
            if "fill" in self.widget.props:
                ...
                if "fillAlarm" in self.widget.props:
                    ...
                    "lineColor" in self.widget.props
                    and "fillColor" in self.widget.props
    ...
    def samePoints(self, propname):
        ...
        return (
            abs(
                int(self.widget.props[propname]["0"])
                - int(self.widget.props[propname]["1"])
            )
            <= 2
        )
