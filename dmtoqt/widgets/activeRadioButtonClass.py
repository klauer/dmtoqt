...
class activeRadioButtonClass(BaseWidget):
    ...
    def __init__(self, widget):
        ...
        if self.framework() == "EpicsQt":
            ...
            if w < 120 or h < 40:
                self.logger.warn(
                    'QERadioGroup "%s": Size (%dx%d) is less than the minimum (%dx%d).  Please examine this element in Qt Designer and adjust as necessary.'
                    % (self.widgetName(), w, h, 120, 40)
                )
            ...
            self.logger.warn(
                'QERadioGroup "%s": Height has been adjusted from %d to %d to make room for the title.  Please examine this element in Qt Designer and adjust as necessary.'
                % (self.widgetName(), h, h + 20)
            )
    ...
    def widgetUI(self, parent):
        ...
        lbl = "_"
        if (
            "controlPv" in self.widget.props
            and len(self.widget.props["controlPv"].strip()) > 0
        ):
