...
argparser = argparse.ArgumentParser(
    description="dmtoqt (version %s): Convert EDM files to epicsqt-aware Qt ui files (for more details, please visit %s)"
    % (v, docurl)
)
...
argparser.add_argument(
    "-p",
    "--path",
    help="Path in which to look for input files and write output files",
    default=os.getcwd(),
)
argparser.add_argument(
    "-c",
    "--colors",
    help="Path to colors.list file.  If not provided, will look in $EDMOBJECTS, $EDMPVOBJECTS, /etc/edm/edmobjects",
)
argparser.add_argument(
    "-f",
    "--framework",
    help='Use framework "EpicsQt" and/or "caQtDM".  If only one argument is provided, only that framework will be used; if both are specified the best match for each widget will be chosen. Default is to use EpicsQt only',
    nargs="+",
    default=["EpicsQt"],
)
argparser.add_argument(
    "file",
    help="File(s) to convert.  Glob-style wildcards may be used.  If no extension is provided, .edl will be added.  Output goes to {file}.ui",
    nargs="+",
)
...
logging.basicConfig(
    format="%(asctime)s %(levelname)s %(message)s", level=level, stream=sys.stdout
)
...
for fnpattern in args.file:
    ...
    for fname in glob.iglob(os.path.join(args.path, fnpattern)):
        ...
        if idx < 0:
            ...
            logger.warn(
                "Period found in filename %s but does not seem to indicate an extension (length = %d); adding one"
                % (f, len(fname) - idx)
            )
            fname = fname + ".edl"
        ...
        try:
            ...
            logger.exception("Exception parsing file %s: %s", fname, ex)
...
logger.info(
    "Finished; %d success, %d failed, %d total"
    % (success, len(args.file) - success, len(args.file))
)
if len(failures) > 0:
