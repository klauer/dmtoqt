...
class DMReader:
        ...
        for line in infile:
            ...
            if m is not None:
                ...
                self.logger.critical(
                    'Unrecognized string on line %d: "%s"' % (self.lineno, line)
                )
                return False
        self.logger.info(
            "Found %d widget(s) in %d lines" % (len(self.widgets), self.lineno)
        )
        ...
        for line in infile:
            ...
            if line == "beginGroup":
                if type != "activeGroupClass":
                    self.logger.critical(
                        "[line %d]: beginGroup in unexpected location; giving up"
                        % self.lineno
                    )
                    return None
                ...
                for line in infile:
                    ...
                    if line == "endGroup":
                        ...
                        self.logger.critical(
                            "[line %d]: endGroup in unexpected location; giving up"
                            % self.lineno
                        )
                        return None
                    m = DMReader._re_objdecl.match(line)
        ...
        for widget in widgets:
            if widget.type == "activeGroupClass":
                ...
                if not self.findRedundantWidgets(widget):
                    self.logger.critical(
                        "findRedundantWidgets failed for an activeGroupClass"
                    )
                    return False
                continue
            if widget.type in ["activeBarClass", "activeMeterClass"]:
                for widget2 in widgets:
                    if widget2.type == "activeXTextDspClass" and self.overlapping(
                        widget, widget2
                    ):
                        ...
                        pv = ""
                        if (
                            widget.type == "activeBarClass"
                            and "indicatorPv" in widget.props
                        ):
                            pv = widget.props["indicatorPv"]
                        elif (
                            widget.type == "activeMeterClass"
                            and "readPv" in widget.props
                        ):
                            pv = widget.props["readPv"]
                        if pv != pv2:
                            continue
                        self.logger.info(
                            "Found a text monitor overlapping a graphical widget that can replace the text; removing the text monitor (pv = %s)"
                            % pv
                        )
                ...
                for widget2 in widgets:
                    if widget2.type == "activeXTextClass" and self.overlapping(
                        widget, widget2
                    ):
                        widget.props["buttonLabel"] = "\n".join(
                            list(widget2.props["value"].values())
                        )
                        ...
                        self.nRemoved += 1
                        self.logger.info(
                            'Found a static text overlapping a button; removing the static text "%s" and setting the button text'
                            % widget.props["buttonLabel"]
                        )
                        break
        if parent is None and self.nRemoved > 0:
            self.logger.info(
                "Removed %d text monitor widgets as they were redundant" % nRemoved
            )
        return True
    ...
    def findStackedWidgets(self, parent=None):
        ...
        for widget in widgets:
            ...
            if widget.type == "activeGroupClass":
                ...
                if not self.findStackedWidgets(widget):
                    self.logger.critical(
                        "findStackedWidgets failed for an activeGroupClass"
                    )
                    return False
                continue
            wtypes = ["activeXTextClass", "activeCircleClass", "activeRectangleClass"]
            if widget.type in wtypes and (
        ...
        if parent is None and self.nStacks > 0:
            self.logger.info(
                "Combined %d widgets into %d stacks" % (self.nWidgets, self.nStacks)
            )
        return True
    ...
    def overlapping(self, widget, widget2):
        ...
        if (
            abs(hmin1 - hmin2) > 10
            or abs(vmin1 - vmin2) > 10
            or abs(vmax1 - vmax2) > 10
        ):
            return False
