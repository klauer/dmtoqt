...
class activeMeterClass(BaseWidget):
    ...
    def widgetUI(self, parent):
        ...
        if "scaleLimitsFromDb" not in self.widget.props:
            ...
            if (
                len(
                    set(self.widget.props)
                    & set(["labelIntervals", "majorIntervals", "minorIntervals"])
                )
                > 0
            ):
                min = 0.0
                ...
                self.logger.info(
                    "%s %s: minorInterval=%g; majorInterval=%g"
                    % (self.widgetType(), self.widgetName(), smallintvl, bigintvl)
                )
        else:
            self.logger.warn(
                "%s %s: scaleLimitsFromDb is set in .edl file.  This does not always work correctly; please check minorInterval and majorInterval"
        ...
        if "labelType" in self.widget.props:
            ...
            self.stringProperty(
                elem, "localEnumeration", '<=0:"%s",>0:"%s"' % (label, label)
            )
