...
class BaseWidget(object):
    ...
    def colorProperty(self, elem, name, value):
        ...
        if isinstance(rgb, str):
            self.logger.warn(
                'Color returned from getRGB is a string "%s"; setting property %s as a string'
                % (rgb, name)
            )
    ...
    def fontProperty(self, elem, name, value, topName=None, force=False):
        ...
        if m is None:
            self.logger.warn(
                'Property %s: Ignoring unexpected font value: "%s"' % (name, value)
            )
            return None
        family = "Sans Serif"
        if m.group(1) == "helvetica" or m.group(1) == "utopia":
            ...
            self.logger.warn(
                'Property %s: Unrecognized font family, falling back to Serif ("%s")'
                % (name, value)
            )
    ...
    def startWidget(self, parent):
        ...
        Returns:
                lxml.etree.elem: The created XML sub element
        """
        self._widgetelem = etree.SubElement(
            parent, "widget", {"class": self.widgetType(), "name": self.widgetName()}
        )
    ...
    def defaultStyle(
        self,
        parent,
        props={"background-color": "bgColor", "color": "fgColor"},
        pressedprops={},
    ):
    ...
    def visPvUI(self, elem):
        ...
        self.stringProperty(
            elem, "channel", self.widget.props["visPv"], {"notr": "true"}
        )
        min = max = "0"
